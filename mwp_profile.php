...
function mwp_show_profile_fields( $user ) {
	...
	echo "<br><div style='color:red'>" . htmlspecialchars($_SESSION["MWP_API_ERROR"]) ."</div>";
...
function mwp_save_profile_fields( $user_id ) {
	...
	//Save mangopay user
	$user = new mwp\mwp_user ( wp_get_current_user(), false );
	mwp\mwp_user::mwp_save_user ( $user );
	//Save mangopay bankaccount
	$bank = new mwp\mwp_bank ( wp_get_current_user(), false );
	mwp\mwp_bank::mwp_save_bank ( $bank ); 
...
function mwp_display_errors ($translated_text, $untranslated_text, $text){
	...
	if ( $_SESSION["MWP_API_ERROR"] !=  NULL && ( $translated_text == 'Profile updated.' ||  $translated_text == 'User updated.') ) 
		return __ ( 'save_profile_error', 'mangopay_wp_plugin' ) . htmlspecialchars($_SESSION["MWP_API_ERROR"]);
